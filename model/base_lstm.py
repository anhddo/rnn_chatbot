...
def create_model(vocab_size):
    encoder = Encoder(vocab_size, config.hidden_size, n_layers = config.n_encoder_layers, \
            dropout=config.dropout)
    decoder = Decoder(config.hidden_size, vocab_size, n_layers = config.n_decoder_layers, \
            dropout=config.dropout)
    model = Seq2Seq(
        encoder=encoder,
        decoder=decoder,
